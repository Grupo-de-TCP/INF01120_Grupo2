...
export function createShadowColorVar(color: string) {
  return `0 8px 16px 0 rgba(var(--palette-${color}-mainChannel) / 0.24)`;
}
...
export function customShadows(colorScheme: ThemeColorScheme) {
  ...
  return {
    ...
    //
    primary: createShadowColorVar('primary'),
    secondary: createShadowColorVar('secondary'),
    info: createShadowColorVar('info'),
    success: createShadowColorVar('success'),
    warning: createShadowColorVar('warning'),
    error: createShadowColorVar('error'),
  };
}
